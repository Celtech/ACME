...
func (c *Certifier) Obtain(request ObtainRequest) (*Resource, error) {
	...
	if request.Bundle {
		...
		}).Infof("acme: Obtaining bundled SAN certificate")
	} else {
		log.WithFields(log.Fields{
			"domain": strings.Join(domains, ", "),
		}).Infof("acme: Obtaining SAN certificate")
	}
	...
	log.WithFields(log.Fields{
		"domain": strings.Join(domains, ", "),
	}).Infof("acme: Validations succeeded; requesting certificates")
...
func (c *Certifier) ObtainForCSR(request ObtainForCSRRequest) (*Resource, error) {
	...
	if request.Bundle {
		log.WithFields(log.Fields{
			"domain": strings.Join(domains, ", "),
		}).Infof("acme: Obtaining bundled SAN certificate given a CSR")
	} else {
		log.WithFields(log.Fields{
			"domain": strings.Join(domains, ", "),
		}).Infof("acme: Obtaining SAN certificate given a CSR")
	}
...
func (c *Certifier) checkResponse(order acme.ExtendedOrder, certRes *Resource, bundle bool, preferredChain string) (bool, error) {
	...
	if preferredChain == "" {
		...
		}).Infof("Server responded with a certificate.")
	...
	for link, cert := range certs {
		...
		if ok {
			log.WithFields(log.Fields{
				"domain": certRes.Domain,
			}).Infof("Server responded with a certificate for the preferred certificate chains %q.", preferredChain)
