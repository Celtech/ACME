	...
	mux.HandleFunc(path, func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodGet && s.matcher.matches(r, domain) {
			...
			log.WithFields(log.Fields{
				"domain": domain,
			}).Infof("Served key authentication")
		} else {
			log.WithFields(log.Fields{
				"domain": domain,
			}).Warnf("Received request for domain %s with method %s but the domain did not match any challenge. Please ensure you are passing the %s header properly.", r.Host, r.Method, s.matcher.name())
