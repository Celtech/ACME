...
func (c *Challenge) PreSolve(authz acme.Authorization) error {
	...
	log.WithFields(log.Fields{
		"domain": domain,
	}).Infof("acme: Preparing to solve DNS-01")
...
func (c *Challenge) Solve(authz acme.Authorization) error {
	...
	log.WithFields(log.Fields{
		"domain": domain,
	}).Infof("acme: Trying to solve DNS-01")
	...
	log.WithFields(log.Fields{
		"domain": domain,
	}).Infof("acme: Checking DNS record propagation using %+v", recursiveNameservers)
	...
	err = wait.For("propagation", timeout, interval, func() (bool, error) {
		...
		if !stop || errP != nil {
			log.WithFields(log.Fields{
				"domain": domain,
			}).Infof("acme: Waiting for DNS record propagation.")
		}
		return stop, errP
	})
	if err != nil {
...
func (c *Challenge) CleanUp(authz acme.Authorization) error {
	log.WithFields(log.Fields{
		"domain": challenge.GetTargetedDomain(authz),
	}).Infof("acme: Cleaning DNS-01 challenge")
